...
CHECKOUT_HANDLER_FOLDER=CheckoutHandler
SOURCE_FILE=main.go

.PHONY: help build-webhook build-user-auth build-user-login build-checkout-handler zip-webhook zip-user-auth zip-user-login zip-checkout-handler test-webhook-api-gateway test-auth-layer
.DEFAULT_GOAL := help
...
help:
	...
	@echo "  make build-checkout-handler    # Build the Checkout Handler"
	...
	@echo "  make zip-checkout-handler      # Zip the Checkout Handler for deployment"
...
build-checkout-handler:
	@echo "Building the Checkout Handler..."
	cd $(CHECKOUT_HANDLER_FOLDER) && \
	GOARCH=$(GOARCH) GOOS=$(GOOS) go build -o $(LAMBDA_BINARY) $(SOURCE_FILE) && \
	cd ..
...
zip-checkout-handler: build-checkout-handler
	@echo "Zipping the Checkout Handler..."
	cd $(CHECKOUT_HANDLER_FOLDER) && \
	zip $(ZIP_FILE) $(LAMBDA_BINARY) && \
	cd ..
